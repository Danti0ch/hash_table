...
Читатель должен иметь базовые знания о том, что такое хэш-таблица, хэш-функция, что такое fill factor хэш таблицы.

Ниже будет описываться реализация, оптимизация и анализ хэш таблицы, реализованной с помощью метода цепочек. Хэш таблица будет хранить пары: ключ-значение. В качестве ключей выступают строки, в качестве значений - числа типа int. Если длина ключа <= 32, то ключ хранится в буфере. Под каждый ключ выделено 32 байта в буфере. Или если длина ключа превышает 32, то под каждый ключ выделяется память, то есть ключ хранится отдельно.

![изображение](https://user-images.githubusercontent.com/89589647/167500189-067ee293-473b-4731-a907-8a5371a44ea4.png)

Основные функции это функции создания таблицы, удаления таблицы, вставки ключа в таблице и проверку нахождения ключа в таблице. Хэш функция - crc32. 
Размер таблицы меняется когда fill-factor превышает константу(значение 0.6).
...
### Оптимизации
Идея юзкейса:  предварительная загрузка данных в таблицу, а потом операции нахождения и обновления элемента таблицы. Будем использовать таблицу так: допустим мы хотим узнать частоту нахождения слов из файла-словаря в каком-то другом файле. Возьмем файл-словарь D, загрузим из него слова в таблицу и будем считать частоту появления слов из словаря D в большом файле F. То есть слова, которые присутствует в файле F, и отстутствуют в файле D добавляться в таблицу не будут. Будем оценивать производительность стадии подсчета и обновления частоты. Так как обновление не будет приводить к добавлению элементов, то не будут затрагиваться части HTableInsert, которых бы не было в HTableFind.
