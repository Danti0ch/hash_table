...
Ниже будет описываться реализация, оптимизация и анализ хэш таблицы, реализованной с помощью цепочек. Хэш таблицы будет хранить пары - ключ-значение. В качестве ключей выступают строки, в качестве значений - числа типа int. Если длина ключа <= 32, то ключ хранится в буфере. Под каждый ключ выделено 32 байта в буфере. Или если длина ключа превышает 32, то под каждый ключ выделяется память.
