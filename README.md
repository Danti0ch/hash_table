# hash_table
Реализация, оптимизация и анализ хэш таблицы с закрытой адресацией. Значения хэш таблицы - значения int. Ключи - строки. Ключи хранятся в буфере в ячейках по 32 байта. А адреса строк в буфере хранятся в массиве списков. Или если длина ключа превышает 32, то под каждый ключ выделяется память и ключ хранится в отдельном списке. Основные функции это функции создания таблицы, удаления таблицы, вставки ключа в таблице и проверку нахождения ключа в таблице. Хэш функция - crc32.
...
Также был сделан замер по результатам которого на выполнение функции LoadHTable ушло 464 * 10^7 тактов процессора(медиана из 64 тестов).
...
Теперь на выполнение LoadHTable уходит 390 * 10^7 тактов. Время выполнения программы уменьшилось на 16%.
...
В дальнейшем мы будем оптимизировать именно эти функции. То есть в нагрузочной функции HtableFind, HTableInsert заменятся на HTableFindAligned, HTableInsertAligned.

С ними генеральная функция выполняется уже за 318 * 10^7 тактов

Время выполнения программы уменьшилось на 15%.
...
Генеральная функция выполняется уже за 313 * 10^7 тактов.

Время выполнения программы уменьшилось на 1%.

### итог
Дальнейшие оптимизации не требуются. Во первых последняя оптимизация дала нам немного - 1%. Необходимые примитивные инструкции убрать нельзя, а самые затратные участки кода уже были оптимизированы.

Тем самым мы получили ускорение:
```
4644596385 -> 3131261610 тактов
Или уменьшение времени выполнения на 32%(или увеличение времени работы программы в 1.48 раз).
```
<!---
### доп оптимизация
